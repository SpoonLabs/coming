...
import fr.inria.prophet4j.feature.S4RO.S4ROFeature.AtomicFeature;
import fr.inria.prophet4j.feature.S4RO.S4ROFeature.CrossType;
import fr.inria.prophet4j.feature.S4RO.S4ROFeature.RepairFeature;
import fr.inria.prophet4j.feature.S4RO.S4ROFeature.ValueFeature;
import fr.inria.prophet4j.feature.S4RO.util.S4ROFeatureVisitor;
...
//        }
    ...
    public FeatureVector extractFeature(Repair repair, CtElement atom) {
        ...
        Map<String, Set<AtomicFeature>> dstMap = new S4ROFeatureVisitor(valueExprInfo).traverseRepair(repair, atom);
        ...
        for (Feature repairFeature : repairFeatures) {
            ...
            featureVector.addFeatureCross(new S4ROFeatureCross(CrossType.RF_CT, features));
        }
        ...
        for (Feature repairFeature : repairFeatures) {
            if (srcMapC.containsKey("@")) {
                ...
                for (Feature atomicFeature : atomicFeatures) {
                    ...
                    featureVector.addFeatureCross(new S4ROFeatureCross(CrossType.POS_AF_RF_CT, globalFeatures));
                }
            }
            ...
            if (srcMapC.containsKey(key)) {
                ...
                for (Feature dstAtomicFeature : dstAtomicFeatures) {
                    for (Feature srcAtomicFeature : srcAtomicFeatures) {
                        ...
                        featureVector.addFeatureCross(new S4ROFeatureCross(CrossType.POS_AF_AF_CT, varCrossFeatures));
                    }
                }
            }
        ...
        for (String key : dstMap.keySet()) {
            ...
            for (Feature atomicFeature : atomicFeatures) {
                for (Feature valueFeature : valueFeatures) {
                    ...
                    featureVector.addFeatureCross(new S4ROFeatureCross(CrossType.AF_VF_CT, valueCrossFeature));
                }
            }
        }
