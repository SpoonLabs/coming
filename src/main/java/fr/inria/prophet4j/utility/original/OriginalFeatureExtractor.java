...
import fr.inria.prophet4j.utility.original.util.OriginalFeatureVisitor;
...
//        int pivot = getPivot(srcStmtList, dstStmtList);
        ...
        FeatureManager featureManager = new FeatureManager(FeatureOption.ORIGINAL);
        Value2FeatureMap4Original resv = new Value2FeatureMap4Original();
        Value2FeatureMap4Original resv_loc = new Value2FeatureMap4Original();
        Value2FeatureMap4Original resv_loc1 = new Value2FeatureMap4Original();
        Value2FeatureMap4Original resv_loc2 = new Value2FeatureMap4Original();
        ...
        for (Feature repairFeature : repairFeatures) {
            ...
            featureManager.addFeature(new OriginalFeatureCross(CrossType.RF_CT, features));
        }

        OriginalFeatureVisitor FEV = new OriginalFeatureVisitor(valueExprInfo);
        ...
        for (CtElement it : loc_stmts) {
            FEV = new OriginalFeatureVisitor(valueExprInfo);
            FEV.traverseStmt(it);
            Value2FeatureMap4Original resM = FEV.getFeatureResult();
            orMap(resv_loc, resM);
        }
        ...
        for (Feature repairFeature : repairFeatures) {
            if (resv_loc.map.containsKey("")) {
                ...
                for (Feature atomicFeature : atomicFeatures) {
                    ...
                    featureManager.addFeature(new OriginalFeatureCross(CrossType.POS_AF_RF_CT, globalFeatures));
                }
            }
            ...
            if (resv_loc.map.containsKey(key)) {
                ...
                for (Feature dstAtomicFeature : dstAtomicFeatures) {
                    for (Feature srcAtomicFeature : srcAtomicFeatures) {
                        ...
                        featureManager.addFeature(new OriginalFeatureCross(CrossType.POS_AF_AF_CT, varCrossFeatures));
                    }
                }
            }
        ...
        for (String key : resv.map.keySet()) {
            ...
            for (Feature atomicFeature : atomicFeatures) {
                for (Feature valueFeature : valueFeatures) {
                    ...
                    featureManager.addFeature(new OriginalFeatureCross(CrossType.AF_VF_CT, valueCrossFeature));
                }
            }
        }
    ...
    private void orMap(Value2FeatureMap4Original m1, final Value2FeatureMap4Original m2) {
