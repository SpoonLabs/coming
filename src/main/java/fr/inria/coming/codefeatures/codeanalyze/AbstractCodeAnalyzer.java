...
public abstract class AbstractCodeAnalyzer {
	...
	public void writeGroupedInfo(Cntx<Object> context, String key, CodeFeatures property, Boolean value,
			String type) {
...
//							.collectVariableReadIgnoringBlocks(aStatement);
	...
	public boolean analyze_AffectedFieldAssigned (List<CtVariableAccess> varsAffected, CtElement element, CtClass parentClass) {
			...
			if(!(S9anyhasNormalGuard && S10anyhasNULLGuard)) {
              for (CtConstructorCall constructorcall : constructorcallstostudy) {
				...
				for (CtConstructorCall specificconstructorcall : allconstructorcallsFromClass) {
					...
					if(getSimplenameForConstructorCall(constructorcall).equals
							(getSimplenameForConstructorCall(specificconstructorcall))) {
	...
	public CtInvocation checkInvocationWithParemetrCompatibleType (List<CtInvocation> invocationsFromClass,
	 ...
	 public List<CtTypeReference> inferPotentionalTypes (CtInvocation ainvocation,
				CtClass parentclass) {
