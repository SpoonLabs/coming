...
//				elementinfo.invocations, elementinfo.constructorcallsFromClass, elementinfo.constructorcalls);
		...
		analyzeS16_HasInvocationsPronetoException(elementinfo.element, elementinfo.context, elementinfo.invocations, elementinfo.constructorcalls);
		
	//	analyzeS17_HasExceptionImport(elementinfo.context, elementinfo.parentClass, elementinfo.invocations);
...
//		}
	...
	private void analyzeS16_HasInvocationsPronetoException (CtElement element, Cntx<Object> context, List<CtInvocation> invocationstostudy
				   ...
				   for (CtInvocation invocation : invocationstostudy) {	
					  ...
					  if(!name.isEmpty() && name!=null) {
						...
						if(name.toLowerCase().indexOf("close")!=-1 || name.toLowerCase().indexOf("connection")!=-1 ||
						   name.toLowerCase().indexOf("stall")!=-1 || name.toLowerCase().indexOf("accept")!=-1 || 
						   name.toLowerCase().indexOf("context")!=-1 || name.toLowerCase().indexOf("write")!=-1 ||
						   name.toLowerCase().indexOf("getmethod")!=-1 || name.toLowerCase().indexOf("getconstructor")!=-1
						   || name.toLowerCase().indexOf("getdeclaredmethod")!=-1 || name.toLowerCase().indexOf("getdeclaredfield")!=-1
						   || name.toLowerCase().indexOf("read")!=-1 || name.toLowerCase().indexOf("open")!=-1 ||
						   name.toLowerCase().indexOf("resource")!=-1 || name.toLowerCase().indexOf("parse")!=-1 ||
						    name.toLowerCase().indexOf("waitfor")!=-1 || name.toLowerCase().indexOf("install")!=-1
						    || name.toLowerCase().indexOf("load")!=-1 || name.toLowerCase().indexOf("synchron")!=-1
						    || name.toLowerCase().indexOf("flush")!=-1 || name.toLowerCase().indexOf("listen")!=-1
						    || name.toLowerCase().indexOf("invoke")!=-1 || name.toLowerCase().indexOf("clone")!=-1 
						    || name.toLowerCase().indexOf("shutdown")!=-1 ||name.toLowerCase().indexOf("connect")!=-1) {
				      ...
				      for (CtConstructorCall constructorcall : constructorcalls) {
					    ...
					    if(!name.isEmpty() && name!=null) {
							...
							if(name.toLowerCase().indexOf("stream")!=-1 || name.toLowerCase().indexOf("file")!=-1 ||
							   name.toLowerCase().indexOf("output")!=-1 || name.toLowerCase().indexOf("accept")!=-1 ||
    ...
    private void analyzeFeature_ExtendFromVar (Cntx<Object> context, List<CtVariableAccess> varsAffected) {
			...
			context.put(CodeFeatures.S5_SIMILAR_PRIMITIVE_TYPE_WITH_NORMAL_GUARD, S5_SIMILAR_PRIMITIVE_TYPE_WITH_NORMAL_GUARD);
    ...
    private void analyzeFeature_ExtendFromMethod (Cntx<Object> context, List<CtInvocation> invocations,  List<CtConstructorCall> constructorcalls) {
			...
			try {	
			    ...
			    if(featuresMethod!=null) {
					...
					if(particularMethod!=null) {
					    ...
					    if(Boolean.valueOf(particularMethod.getInformation().get("M9_METHOD_CALL_WITH_NORMAL_GUARD").toString()))
					    	S9_METHOD_CALL_WITH_NORMAL_GUARD = true;
					
					    if(Boolean.valueOf(particularMethod.getInformation().get("M10_METHOD_CALL_WITH_NULL_GUARD").toString()))
					    	S10_METHOD_CALL_WITH_NULL_GUARD = true;
					
					    if(Boolean.valueOf(particularMethod.getInformation().get("M12_METHOD_CALL_WITH_TRY_CATCH").toString()))
			...
			try {	
			    ...
			    if(featuresConstructor!=null) {
					...
					if(particularConstructor != null) {
					    ...
					    if(Boolean.valueOf(particularConstructor.getInformation().get("CON9_METHOD_CALL_WITH_NORMAL_GUARD").toString()))
					    	S9_METHOD_CALL_WITH_NORMAL_GUARD = true;
					
					    if(Boolean.valueOf(particularConstructor.getInformation().get("CON10_METHOD_CALL_WITH_NULL_GUARD").toString()))
					    	S10_METHOD_CALL_WITH_NULL_GUARD = true;
					
					    if(Boolean.valueOf(particularConstructor.getInformation().get("CON12_METHOD_CALL_WITH_TRY_CATCH").toString()))
					    	S12_METHOD_CALL_WITH_TRY_CATCH = true;
					}
			    }
			} catch (Throwable e) {
