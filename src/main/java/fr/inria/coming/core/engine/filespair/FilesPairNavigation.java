...
/**
 * This class implements the `filespair` input type/format in coming.
 * Here, -input is `filespiar` and -location is supposed be in the format of `source_file_path:target_file_path`.
 * The mentioned analysis will be performed on revision from the source_file to the target_file.
 *
...
public class FilesPairNavigation extends RevisionNavigationExperiment<IRevision> {
    ...
    public FilesPairNavigation() {
        ...
        if (paths.length != 2)
            throw new IllegalArgumentException("Paths to two files required. Give: " + paths.length + " in \"" + locationArg + "\"");

        leftFile = new File(paths[0]);
        rightFile = new File(paths[1]);

        if (!leftFile.exists())
            throw new IllegalArgumentException("Location does not exist: " + leftFile);
        if (!rightFile.exists())
            throw new IllegalArgumentException("Location does not exist: " + leftFile);
    ...
    public RevisionDataset<IRevision> loadDataset() {
        ...
        /*
        create a dummy list(of size 1) so that it can be used as a list of revisions(i.e 1 in this case)
        for creating a dummy `RevisionDataset` and  `RevisionOrder`.
        */
...
/**
 * A Dummy RevisionOrder required to set the NavigationStrategy..
 * The iterator returned by orderOfNavigation will always contain just one revision.
 */
