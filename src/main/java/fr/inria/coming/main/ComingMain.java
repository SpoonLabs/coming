...
import fr.inria.coming.repairability.RepairabilityFilterAnalyzer;
...
public class ComingMain {
    ...
    static {
        ...
        options.addOption(Option.builder("input").argName("git(default) | files | filespair | repairability").hasArg().desc(
    ...
    private void loadModelAnalyzers(String modes) {
        ...
        for (String mode : modesp) {
            ...
            if ("diff".equals(mode)) {
                ...
                navigatorEngine.getAnalyzers().add(new PatternInstanceAnalyzer(patterns));
                navigatorEngine.getAnalyzers().add(new RepairabilityFilterAnalyzer());
