 ...
 * 
 * <P>
 * This is Derby's implementation of a PooledConnection for use in the following
 * environments:
 * <UL>
...
class EmbedPooledConnection implements javax.sql.PooledConnection, BrokeredConnectionControl {
	...
	/**
	 * The number of iterators going through the list of connection event listeners
	 * at the current time. Only one thread may be iterating over the list at any
	 * time (because of synchronization), but a single thread may have multiple
	 * iterators if for instance an event listener performs database calls that
	 * trigger a new event.
	 */
	...
	/**
	 * True if the password was passed in on the connection request, false if it
	 * came from the data source property.
	 */
	...
	final Connection getNewCurrentConnectionHandle() {
		Connection applicationConnection = currentConnectionHandle = ((org.apache.derby.jdbc.Driver20) (realConnection
				.getLocalDriver())).newBrokeredConnection(this);
	...
	void resetRealConnection() throws SQLException {
		...
		// user are rolledback.
		//realConnection.rollback();deleted
	...
	/**
	 * Reset the isolation level flag used to keep state in BrokeredConnection. It
	 * will get set to true when isolation level is set using JDBC/SQL. It will get
	 * reset to false at the start and the end of a global transaction.
	 */
	...
	/**
	 * Notify the control class that a SQLException was thrown during a call on one
	 * of the brokered connection's methods.
	 */
	...
	/**
	 * Close called on BrokeredConnection. If this call returns true then
	 * getRealConnection().close() will be called.
	 * 
	 * 
	 * Notify listners that connection is closed. Don't close the underlying real
	 * connection as it is pooled.
	 */
	 ...
	 * EmbedPreparedStatement class to set the BrokeredConnectionControl variable to
	 * this instance of EmbedPooledConnection This will then be used to call the
	 * onStatementErrorOccurred and onStatementClose events when the corresponding
	 * events occur on the PreparedStatement
	 *
	...
	/**
	 * Call the setBrokeredConnectionControl method inside the
	 * EmbedCallableStatement class to set the BrokeredConnectionControl variable to
	 * this instance of EmbedPooledConnection This will then be used to call the
	 * onStatementErrorOccurred and onStatementClose events when the corresponding
	 * events occur on the CallableStatement
	 *
	 ...
	 *
	 * A pooled connection is assigned a separate id from a physical connection.
	 * When a container calls PooledConnection.toString(), it gets the string
	 * representation of this id. This is useful for developers implementing
	 * connection pools when they are trying to debug pooled connections.
	 *
	 * @return a string representation of the uniquie id for this pooled connection.
	 *
	 */
	public String toString() {
		if (connString == null) {
			String physicalConnString = isActive ? realConnection.toString() : "<none>";

			connString = this.getClass().getName() + "@" + this.hashCode() + " " + "Physical Connection = "
					+ physicalConnString;
		}
	...
	/*-----------------------------------------------------------------*/
	/*
	 * These methods are from the BrokeredConnectionControl interface. These methods
	 * are needed to provide StatementEvent support for derby. They are actually
	 * implemented in EmbedPooledConnection40 but have a dummy implementation here
	 * so that the compilation wont fail when they are compiled with jdk1.4
	 */
