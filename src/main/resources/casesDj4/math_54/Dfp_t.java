 ...
 *
 * <p>
 * Another floating point class. This one is built using radix 10000 which is
 * 10<sup>4</sup>, so its almost decimal.
 * </p>
 *
 * <p>
 ...
 * <li>Settable precision (but no mix between numbers using different
 * settings)</li>
 * <li>Portability. Code should be keep as portable as possible.</li>
 * <li>Performance</li>
 * <li>Accuracy - Results should always be +/- 1 ULP for basic algebraic
 * operation</li>
 * <li>Comply with IEEE 854-1987 as much as possible. (See IEEE 854-1987 notes
 * below)</li>
 * </ol>
 * </p>
 *
 ...
 * Trade offs:
 * <ol>
 * <li>Memory foot print. I'm using more memory than necessary to represent
 * numbers to get better performance.</li>
 * <li>Digits are bigger, so rounding is a greater loss. So, if you really need
 * 12 decimal digits, better use 4 base 10000 digits there can be one partially
 * filled.</li>
 * </ol>
 * </p>
 *
 ...
 * </pre>
 * 
 * where sign is &plusmn;1, mantissa represents a fractional number between zero
 * and one. mant[0] is the least significant digit. exp is in the range of
 * -32767 to 32768
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * IEEE 854 requires the radix to be either 2 or 10. The radix here is 10000, so
 * that requirement is not met, but it is possible that a subclassed can be made
 * to make it behave as a radix 10 number. It is my opinion that if it looks and
 * behaves as a radix 10 number then it is one and that requirement would be
 * met.
 * </p>
 *
 * <p>
 * The radix of 10000 was chosen because it should be faster to operate on 4
 * decimal digits at once instead of one at a time. Radix 10 behavior can be
 * realized by add an additional rounding step to ensure that the number of
 * decimal digits represented is constant.
 * </p>
 *
 * <p>
 * The IEEE standard specifically leaves out internal data encoding, so it is
 * reasonable to conclude that such a subclass of this radix 10000 system is
 * merely an encoding of a radix 10 system.
 * </p>
 *
 * <p>
 * IEEE 854 also specifies the existence of "sub-normal" numbers. This class
 * does not contain any such entities. The most significant radix 10000 digit is
 * always non-zero. Instead, we support "gradual underflow" by raising the
 * underflow flag for numbers less with exponent less than expMin, but don't
 * flush to zero until the exponent reaches MIN_EXP-digits. Thus the smallest
 * number we can represent would be: 1E(-(MIN_EXP-digits-1)*4), eg, for
 * digits=5, MIN_EXP=-32767, that would be 1e-131092.
 * </p>
 *
 * <p>
 * IEEE 854 defines that the implied radix point lies just to the right of the
 * most significant digit and to the left of the remaining digits. This
 * implementation puts the implied radix point to the left of all digits
 * including the most significant one. The most significant digit here is the
 * one just to the right of the radix point. This is a fine detail and is really
 * only a matter of definition. Any side effects of this can be rendered
 * invisible by a subclass.
 * </p>
 * 
 * @see DfpField
...
public class Dfp implements FieldElement<Dfp> {
	...
	/**
	 * The minimum exponent before underflow is signaled. Flush to zero occurs at
	 * minExp-DIGITS
	 */
	...
	protected Dfp(final DfpField field, final String s) {
		...
		// First off, find all of the leading zeros, trailing zeros, and significant
		// digits
		p = 0;
	...
	/**
	 * Create an instance with a value of 0. Use this internally in preference to
	...
	/**
	 * Create an instance by copying an existing one. Use this internally in
	 * preference to constructors to facilitate subclasses.
	 * 
	 * @param d instance to copy
	...
	/**
	 * Create an instance from a String representation. Use this internally in
	 * preference to constructors to facilitate subclasses.
	 * 
	 * @param s string representation of the instance
	...
	/**
	 * Get the {@link org.apache.commons.math.Field Field} (really a
	 * {@link DfpField}) to which the instance belongs.
	 * <p>
	 * The field is linked to the number of digits and acts as a factory for
	 * {@link Dfp} instances.
	 * </p>
	 * 
	 * @return {@link org.apache.commons.math.Field Field} (really a
	 *         {@link DfpField}) to which the instance belongs
	 */
	...
	/*
	 * Note that shiftRight() does not call round() as that round() itself uses
	 * shiftRight()
	 */
	/**
	...
	/**
	 * Make our exp equal to the supplied one, this may cause rounding. Also causes
	 * de-normalized numbers. These numbers are generally dangerous because most
	 * routines assume normalized numbers. Align doesn't round, so it will return
	...
	protected int align(int e) {
		...
		for (int i = 0; i < adiff; i++) {
			if (diff < 0) {
				/*
				 * Keep track of loss -- only signal inexact after losing 2 digits. the first
				 * lost digit is returned to add() and may be incorporated into the result.
				 */
				if (lostdigit != 0) {
					inexact = true;
				}
	 ...
	 * 
	 * @param x number to check instance against
	 * @return true if instance is greater than x and neither are NaN, false
	 *         otherwise
	 */
	 ...
	 * 
	 * @param x number to check instance against
	 * @return true if instance is not equal to x and neither are NaN, false
	 *         otherwise
	 */
	 ...
	 * @return -1 if a<b, 1 if a>b and 0 if a==b Note this method does not properly
	 *         handle NaNs or numbers with different precision.
	 */
	...
	/**
	 * Round to nearest integer using the round-half-even method. That is round to
	 * nearest integer unless both are equidistant. In which case round to the even
	 * one.
	 * 
	...
	/**
	 * Round to an integer using the round ceil mode. That is, round toward
	 * +Infinity
	 * 
	 * @return rounded value
	 */
	...
	public Dfp remainder(final Dfp d) {
		...
		// IEEE 854-1987 says that if the result is zero, then it carries the sign of
		// this
	...
	protected Dfp trunc(final DfpField.RoundingMode rmode) {
		...
		/*
		 * If the exponent is less than zero then we can certainly return zero
		 */
		if (exp < 0) {
			field.setIEEEFlagsBits(DfpField.FLAG_INEXACT);
		...
		/*
		 * If the exponent is greater than or equal to digits, then it must already be
		 * an integer since there is no precision left for any fractional part
		 */
		...
		/*
		 * General case: create another dfp, result, that contains the a with the
		 * fractional part lopped off.
		 */
	...
	/**
	 * Get the exponent of the greatest power of 10000 that is less than or equal to
	 * the absolute value of this. I.E. if this is 10<sup>6</sup> then log10K would
	 * return 1.
	 * 
	...
	/**
	 * Get the exponent of the greatest power of 10 that is less than or equal to
	 * abs(this).
	 * 
	...
	/**
	 * Negate the mantissa of this by computing the complement. Leaves the sign bit
	 * unchanged, used internally by add. Denormalized numbers are handled properly
	 * here.
	 * 
	 * @param extra ???
	 * @return ???
	...
	public Dfp add(final Dfp x) {
		...
		/*
		 * handle overflow -- note, when asign!=bsign an overflow is normal and should
		 * be ignored.
		 */
	...
	/**
	 * Multiply this by a single digit 0&lt;=x&lt;radix. There are speed advantages
	...
	public Dfp divide(Dfp divisor) {
		...
		/*
		 * copy our mantissa into the dividend, initialize the quotient while we are at
		 * it
		 */
		...
		for (qd = mant.length + 1; qd >= 0; qd--) {
			...
			while (!trialgood) {
				...
				/*
				 * May have a good one here, check more thoroughly. Basically its a good one if
				 * it is less than the divisor
				 */
	...
	/**
	 * Divide by a single digit less than radix. Special case, so there are speed
	 * advantages. 0 &lt;= divisor &lt; radix
	 * 
	 * @param divisor divisor
	 * @return quotient of this by divisor
	 */
	...
	public Dfp sqrt() {
		...
		/*
		 * Now that we have the first pass estimate, compute the rest by the formula dx
		 * = (y - x*x) / (2x);
		 */
	...
	/**
	 * Trap handler. Subclasses may override this to provide trap functionality per
	 * IEEE 854-1987.
	 *
	 ...
	 * @param result The result that is specified to be delivered per IEEE 854, if
	 *               any
	...
	public double toDouble() {
		...
		/*
		 * Find the exponent, first estimate by integer log10, then adjust. Should be
		 * faster than doing a natural logarithm.
		 */
