...
public class SubLine extends AbstractSubHyperplane<Euclidean2D, Euclidean1D> {
	 ...
	 * one segment, and each segment contains a start point at index 0 and an end
	 * point at index 1. If the sub-line is unbounded in the negative infinity
	 * direction, the start point of the first segment will have infinite
	 * coordinates. If the sub-line is unbounded in the positive infinity direction,
	 * the end point of the last segment will have infinite coordinates. So a
	 * sub-line covering the whole line will contain just one row and both elements
	 * of this row will have infinite coordinates. If the sub-line is empty, the
	...
	public SplitSubHyperplane<Euclidean2D> split(final Hyperplane<Euclidean2D> hyperplane) {
		...
		if (crossing == null) {
			// the lines are parallel
			final double global = otherLine.getOffset(thisLine);
			return (global < -1.0e-10) ? new SplitSubHyperplane<Euclidean2D>(null, this)
		...
		final BSPTree<Euclidean1D> plusTree = getRemainingRegion().isEmpty(splitTree.getPlus())
				...
				: new BSPTree<Euclidean1D>(subPlus, new BSPTree<Euclidean1D>(Boolean.FALSE), splitTree.getPlus(), null);
		final BSPTree<Euclidean1D> minusTree = getRemainingRegion().isEmpty(splitTree.getMinus())
				? new BSPTree<Euclidean1D>(Boolean.FALSE)
				: new BSPTree<Euclidean1D>(subMinus, new BSPTree<Euclidean1D>(Boolean.FALSE), splitTree.getMinus(),
						null);
